...
def main():
    parser = argparse.ArgumentParser(
        description="سكريپت النشر الموحد - Deploy Script",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
أمثلة الاستخدام:
  %(prog)s                                   # نشر الملفات المعدلة (modified)
  %(prog)s --mode all                        # رفع جميع الملفات
  %(prog)s --mode sync                       # رفع مع تخطي المطابق
  %(prog)s --mode remote                     # فحص ومقارنة الهاش مع السيرفر والرفع
  %(prog)s --mode status                     # عرض حالة التزامن وفحص الملفات
  %(prog)s --mode test                       # اختبار اتصال SSH بالسيرفر
  %(prog)s --mode file --file path/to/file   # رفع ملف أو مسار محدد
        """
    )
